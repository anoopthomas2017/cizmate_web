...
export default {
    ...
    created() {
        // Check for stored credentials when component is created
    ...
    methods: {
        ...
        async saveCredentials() {
            ...
            const encryptedUsername = await encryption.encrypt(this.email);
            const encryptedPassword = await encryption.encrypt(this.password);
        ...
        async loadCredentials() {
            ...
            if (stored) {
                this.email = await encryption.decrypt(stored.username);
                this.password = await encryption.decrypt(stored.password);
            }
        }
    }
...
<template>
    <FloatingConfigurator />
    <form class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw]
        overflow-hidden">
